...
class CartController extends AppController
{
    ...
    public function actionCheckout(){
        parent::seoParams('Confirm Order','','');
        $session=\Yii::$app->session;
        $session->open();
        $order=new Order();
        ...
        if($order->load(\Yii::$app->request->post())){
            ...
            if(!$order->save() || !$order_product->saveOrderProducts($session['cart'],$order->id)){
                echo 'asdouiasgydofsugf';
                \Yii::$app->session->setFlash('error','Error in confirm order!!!');
                $transaction->rollBack();
            }else{
                $transaction->commit();
                \Yii::$app->session->setFlash('success','Success in confirm order!!!');
                 try{
                     \Yii::$app->mailer->compose('order',['session'=>$session])
                         ->setFrom([\Yii::$app->params['senderEmail']=>\Yii::$app->params['senderName']])
                         ->setTo([$order->email,\Yii::$app->params['adminEmail']])
                         ->setSubject('Confirm Order in website Grocery Store')
                         ->send();
